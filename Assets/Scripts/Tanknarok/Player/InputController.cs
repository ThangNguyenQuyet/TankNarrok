	...
	public class InputController : NetworkBehaviour, INetworkRunnerCallbacks
	{
		...
		public void OnInput(NetworkRunner runner, NetworkInput input)
		{
			if (_player!=null && _player.Object!=null && _player.state == Player.State.Active && fetchInput)
			{
				...
				{
					_frameworkInput.Buttons |= NetworkInputData.BUTTON_FORWARD;
				}

				if (Input.GetKey(KeyCode.S))
				{
					_frameworkInput.Buttons |= NetworkInputData.BUTTON_BACKWARD;
				}

				if (Input.GetKey(KeyCode.A))
				{
					_frameworkInput.Buttons |= NetworkInputData.BUTTON_LEFT;
				}

				if (Input.GetKey(KeyCode.D))
				{
					_frameworkInput.Buttons |= NetworkInputData.BUTTON_RIGHT;
				}
				...
				_frameworkInput.aimDirection = CalculateAim();
			}
		...
		public override void FixedUpdateNetwork()
		{
			...
			//if (hit.collider?.gameObject?.layer== LayerMask.NameToLayer("Player")) _player.shooter.FireWeapon(WeaponManager.WeaponInstallationType.PRIMARY);
			//Debug.Log("test");
			//
